...
class DiscoverourmenuDesktop extends StatefulWidget {
  DiscoverourmenuDesktop({Key? key}) : super(key: key);

  @override
  State<DiscoverourmenuDesktop> createState() => _DiscoverourmenuDesktopState();
}

class _DiscoverourmenuDesktopState extends State<DiscoverourmenuDesktop> {
        ...
        child: isloaded 
            ? GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return _buildItemContainer(item, constraints, crossAxisCount);
                },
              ): Center(
                child: CircularProgressIndicator(
                  color: Colors.grey,
...
Widget _buildItemContainer(item, BoxConstraints constraints, int crossAxisCount) {
  return Container(
    decoration: BoxDecoration(
        ...
        FutureBuilder<String>(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              return Center(child: Text('No image found')); // Show error or empty state
            } else {
              try {
                return Image.memory(
                ...
                return Center(child: Text('Error decoding image')); // Handle decoding error
              }
            }
          },
        ),
        Positioned(
          left: constraints.maxWidth * 0.1, // Dynamically adjust left margin based on screen width
          right: 0,
          top: 10,
          bottom: 10,
          child: Container(
            width: crossAxisCount == 1
                ? constraints.maxWidth * 0.8 // Use 80% of available width when only one column
                : constraints.maxWidth * 0.6, // Use 60% of available width when two columns
            height: 50,
...
Future<String> fetchImageFromApi(String itemNumber) async {
  ...
  try {
    ...
    if (responseData.containsKey('Base64String') && responseData['Base64String'].isNotEmpty) {
      ...
      final base64Image = responseData['Base64String'].replaceFirst('data:image/jpg;base64,', '');
      return base64Image;
    } else {
