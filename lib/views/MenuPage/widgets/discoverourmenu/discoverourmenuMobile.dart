...
class DiscoverourmenuMobile extends StatefulWidget {
   final ScrollController scrollController;  // Add this
  const DiscoverourmenuMobile({Key? key, required this.scrollController}) : super(key: key);

  @override
  State<DiscoverourmenuMobile> createState() => _DiscoverourmenuMobileState();
}

class _DiscoverourmenuMobileState extends State<DiscoverourmenuMobile> {
                        ...
                        child: Stack(
                          children: [
                            FutureBuilder<String>(
                              future: fetchImageFromApi(
                                  item.id!), // Fetch image asynchronously
                              builder: (context, snapshot) {
                                if (snapshot.connectionState ==
                                  ...
                                  return Center(
                                      child: Text(
                                          'No image found')); // Show error or empty state
                                } else {
                                  try {
                                    return Image.memory(
                            ...
                            Positioned(
                              left: constraints.maxWidth *
                                  0.35, // Dynamically adjust left margin based on screen width
                              right: 0,
                              top: 10,
                              bottom: 10,
                              child: Container(
        ...
        if (jsonResponse['Department'] is List) {
          ...
          List<dynamic> departmentList = jsonResponse['Department']
              .where((department) =>
                  ...
                  department['DepartmentId'] != 'COUPONS' &&
                  department['DepartmentId'] != 'NO' &&
                  department['DepartmentId'] != 'NONE' &&
                  department['DepartmentId'] != 'ModGroup')
              .toList();
